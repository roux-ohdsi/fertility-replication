...
overweight_obesity                = c(4215968, 4176962, 3025315, 4099154, 3013762, 3027492, 3023166) # also exclude this one: 4264825
# OMOP: Tobacco dependence syndrome
tobacco_use_disorder              = c(437264) # 4103418 is tobacco dependence syndrome in remission - exclude?
...
alcohol_abuse_or_dependence       = c(36714559, 4042860, 435140, 46269816, 37311131, 4058714, 46269817, 195300, 318773, 37016176, 45757783, 44788725, 40484946, 3661461, 44788726, 4261832, 46269818, 4083236)
male_infertility                  = c(198197) # this one is interesting. woudl you also want to include male + infertility? (4311387)
female_infertility                = c(201909) # again - concerned we might miss some infertility codes. can we do infertility + m/f?
female_infertility_unspecified    = c() # this seems like more of an error in their table setup...
...
polycystic_ovarian_syndrome       = c(40443308) 

# ======================== Neonatal outcomes

# Ascertain Window: "From Treatment to Delivery"

multiple_gestation                        =  c(432969) # this one looks pretty good but we should check for an lower-order exclusions

# Ascertain Window: "Maternal and infant claims within 30 days of birth"

small_for_gestational_age                 =  c(72693, 4145947) # 4145947 small for gest. age has higher concept fetal growth restriction - might apply?
large_for_gestational_age                 =  c(434758, 440532, 81636) # first code is exceptionaly large baby. 4079859 is high birth weight. do we want to include?
neonatal_icu_admission                    =  c(42739633,42739550, 42739551, 2514563, 2514564, 2514565, 2514566, 2514569, 2514570, 2514571, 2514572) # CPT codes easy to match
neonatal_icu_admission_infant_record      =  c(2108681, 2108682, 2514441, 2514442) # CPT codes easy to match. these are only if found in the infant record though...
...
gdm_condition_1 = c(2212361, 2212362, 4162865) # 4162865 is for 648.8 - needs manual review to make sure its sufficient/not too broad
# At least one code for DM after 140 gestational days and the delivery. (side note...logical statement is confusing)

gdm_condition_2 = c(201820) # ICD9CM 250.xx - any codes for DM at all. This is just the omop code for general DM then. 

# an absence of DM codes for GDM, DM complicating pregnancy, or DM generally prior to 140 gestational days. 
gdm_condition_2 = c(4162865, 76751001, 201820) # 648.0x maps to 76751001 "Diabetes mellitus in mother complicating pregnancy, childbirth AND/OR puerperium (disorder)"
